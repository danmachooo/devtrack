...
- the dedicated `/tickets` workspace remains the primary project-scoped ticket review and assignment surface, loading through the shared tickets API module, showing mapped status, source status, assignment, assignee, missing state, and sync time, supporting feature/status/unassigned/missing filters, preventing invalid `featureId + unassigned=true` combinations in the UI, and restricting inline assignment to team leaders and business analysts
