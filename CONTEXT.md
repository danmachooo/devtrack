...
- the repo now targets the live backend by default through the shared Axios instance, supports an optional `NEXT_PUBLIC_API_BASE_URL` override for non-default backend hosts, proxies absolute backend URLs back through the app's same-origin `/api` path so session cookies survive split Vercel frontend plus Render backend deployments, keeps mock mode available only through the explicit `NEXT_PUBLIC_USE_MOCK_API=true` escape hatch, and normalizes backend error messages so existing auth, organization, project, notion, sync, feature, ticket, and client-dashboard screens surface API feedback cleanly
- the internal workspace shell now includes a sticky sidebar with Lucide-based navigation, a top header that surfaces active organization context and the signed-in user's account menu, and an icon-led theme toggle backed by the shared token system
...
- the shared `GET /api/projects` contract now includes an additive `progressSummary` per project, and `/dashboard` plus `/projects` read `progressSummary.overallProgress` directly from that single backend response instead of issuing one extra tickets request per project just to reconstruct overview progress in the browser
- the shared `GET /api/projects/:id` contract now also includes additive `progressSummary` data, and the project-detail progress panel reads aggregate and per-feature progress directly from that backend-owned summary instead of issuing separate progress-only feature and ticket queries just to recompute the same result in the browser
...
- the project detail route now loads through the shared projects API module, supports a team-leader-only metadata editor, surfaces readiness and `lastSyncedAt`, keeps a compact setup rail visible, and prioritizes the current incomplete setup step through a focused guided flow instead of rendering every setup and operational panel at full height at once
...
- Notion connection and status-mapping saves now patch project and project-list caches directly, and manual sync now updates cached project freshness metadata immediately while deferring ticket and sync-log refreshes to sync completion, which reduces broad refetch churn around setup and sync actions without changing RBAC or contract behavior
- the project command center now includes a feature management workspace that uses a split-pane editor instead of a long stacked list: a searchable and filterable left pane keeps ordered feature summaries compact, while a focused right pane handles rename, explicit ordering controls, and direct position-based reordering for larger jumps plus stronger client-facing naming guidance; delete confirmation explicitly warns that related ticket assignments will be cleared; and read-only roles can inspect the workspace without management controls in the DOM
- the dedicated `/tickets` workspace remains the primary project-scoped ticket review and assignment surface, loading through the shared tickets API module, showing mapped status, source status, assignment, assignee, missing state, and sync time, supporting backend-driven pagination plus feature/status/unassigned/missing/search/assignee/sort controls, preventing invalid `featureId + unassigned=true` combinations in the UI, supporting selection-based bulk assignment and bulk unassignment for team leaders and business analysts, and continuing to omit assignment controls from the DOM for read-only roles
- ticket assignment interactions now update cached project ticket queries and feature ticket counts directly after row-level and bulk assignment mutations, which keeps the dense `/tickets` workspace responsive without broad project or project-list refetch churn after each edit
- the `/tickets` route now defers the heavy review panel behind a dynamic import so the route shell, project selector, and active-project summary can paint first, and the most repeated ticket workspace subcomponents are memoized to reduce avoidable rerenders during selection and assignment work
...
- the project detail route now uses a hybrid layout where setup-critical steps open in a focused modal-driven flow, the setup rail sits below the overview area in a compact bento-style grid, ticket assignment is handed off into the dedicated `/tickets` workspace, progress plus sync diagnostics move into a lower-priority sharing-and-health area, and sync freshness is represented in the header through a compact hoverable signal instead of a full card
- the project-detail Notion setup now follows a staged flow where team leaders test and save the Notion connection first, then proceed into status mapping only after the connection is valid and saved
...
- the app router now includes route-group `loading.tsx` and `error.tsx` boundaries for internal, auth, and client surfaces, and the `/projects` route now uses a server wrapper that mounts a dedicated client feature component instead of making the route file itself the client entry point
...
- the latest UI polish pass tightened shared header, action, card, modal, popover, and select treatments; improved title wrapping and spacing rhythm across `/tickets`, `/projects`, `/projects/[id]`, and `/organization`; added more purposeful Lucide icon cues to prioritized internal surfaces; and strengthened active or selected visual states without changing workflow order, RBAC behavior, API wiring, or client-safety boundaries
- internal project progress signals now distinguish setup readiness from delivery progress: `/projects` and `/dashboard` show real aggregate progress from the backend-provided `progressSummary`, `/projects/[id]` labels the header card as setup readiness and syncs its count to the visible six-step setup rail, and feature-management rows now show real assigned-ticket progress instead of a placeholder `0%` bar
- the latest interaction-polish pass aligned `/dashboard` with the stronger internal page-header treatment, replaced the dashboard empty-state word badge with a real icon, made `/projects` cards read as full clickable targets, added app-wide pointer cursors for shared clickables, and smoothed the sidebar expand or collapse behavior by animating label width and opacity instead of snapping content in and out
- the project-detail sync signal now acts as both a freshness indicator and a compact action surface: it shows freshness details plus `lastSyncedAt`, exposes manual sync directly from the header for `TEAM_LEADER` and `BUSINESS_ANALYST`, keeps prerequisite messaging visible when Notion connection or mapping are missing, and preserves read-only messaging for other roles
