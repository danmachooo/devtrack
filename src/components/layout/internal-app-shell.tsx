...
export function InternalAppShell({ children }: PropsWithChildren) {
  ...
  return (
    /*
     * Root is a flex row that fills the viewport.
     * overflow-hidden here ensures the sidebar never causes page-level scroll —
     * only the main content column scrolls (via overflow-y-auto below).
     */
    <div className="flex h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
       ...
       *
       * THE ROOT CAUSE FIX:
       * The original layout used `grid-cols-[auto_1fr]` with the aside as
       * `fixed` (mobile) and `sticky` (desktop). Neither `fixed` nor `sticky`
       * elements participate in grid/flex sizing — so the sidebar column had
       * zero width and content started at column 1, getting overlapped.
       *
       * New approach: plain flex row on the root.
       *   • Desktop → aside is a normal `relative` flex child with explicit width.
       *     It naturally pushes the content column to the right. ✓
       *   • Mobile   → aside is `fixed` and slides in as a drawer. The content
       *     column still fills 100% width underneath (correct for mobile). ✓
       */}
      <aside
        className={[
          // Shared structure
          "hide-scrollbar flex shrink-0 flex-col overflow-y-auto border-r border-[var(--border)] bg-[var(--surface)] py-5",
          "transition-[width,padding,transform] duration-200 ease-out",

          // Mobile: fixed drawer, full height, slides in/out
          "fixed inset-y-0 left-0 z-40 w-72 px-4",
          isMobileNavOpen ? "translate-x-0" : "-translate-x-full",

          // Desktop: static flex child — participates in layout so content is pushed right
          "md:relative md:inset-auto md:z-auto md:translate-x-0",
          isSidebarOpen ? "md:w-64 md:px-4" : "md:w-[72px] md:px-2",
        ].join(" ")}
      >
        ...
        <div
          ...
          } bg-[color:color-mix(in_srgb,var(--surface)_94%,transparent)] backdrop-blur`}
        >
          {/* Theme toggle */}
          <button
       ...
       * overflow-y-auto gives this column its own scroll context so the
       * sidebar stays full-height and doesn't scroll with the page.
       */}
      <div className="hide-scrollbar flex min-w-0 flex-1 flex-col overflow-y-auto">
        <header className="border-b border-[var(--border)] bg-[color:color-mix(in_srgb,var(--surface)_78%,transparent)] px-4 py-4 sm:px-6 md:pl-10">
